...
public interface GarageRepository extends MongoRepository<Garage, String> {
    ...
    Garage findGarageByOwnerSurname(String ownerSurname);
    @Query(value = "{'contact.phoneNumber': ?0}")
    Garage findGarageByOwnerPhoneNumber(String phoneNumber);
    @Query(value = "{'car.carNumber': ?0}")
    Garage findGarageByCarNumber(String carNumber);
}
